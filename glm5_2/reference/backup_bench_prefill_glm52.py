...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fixed-stage-name", default="fixed", help="Name for the measured data stage")
    parser.add_argument("--send-token-ids", action="store_true", help="Send prepared token IDs instead of re-encoded prompt text")
    ...
    parser.add_argument(
        "--unique-cache-salt",
        action="store_true",
        help="Use a fresh prefix-cache salt per request to measure uncached prefill",
    )
...
def prepare_prompts(tokenizer, questions: list[str], target: int, send_token_ids: bool = False) -> tuple[list[str | list[int]], list[dict]]:
    prompts = []
    metadata = []
    for index, question in enumerate(questions):
        ...
        prompts.append(selected_ids if send_token_ids else prompt)
        metadata.append({
            "index": index,
            "original_tokens": len(original_ids),
            "prepared_tokens": len(selected_ids) if send_token_ids else round_trip_len,
                ...
                for choice in event.get("choices", []):
                    ...
                    if piece:
                        if first_token_at is None:
                            ...
                            first_token_event = "text"
                            print(f"{stage} {index} first_token ttft={first_token_at - started:.6f}s", flush=True)
                        chunks.append(piece)
                    if choice.get("finish_reason"):
                        if first_token_at is None:
                            first_token_at = time.perf_counter()
                            first_token_event = "finish_event_empty_text"
                            print(f"{stage} {index} first_event ttft={first_token_at - started:.6f}s", flush=True)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as pool:
        futures = [
            pool.submit(
                ...
                unique_cache_salt=args.unique_cache_salt,
            )
...
def main() -> None:
    ...
    if args.fixed_only:
        ...
        warmup, warmup_meta = prepare_prompts(
            tokenizer, load_questions(args.warmup_data, args.warmup_count), args.input_tokens, args.send_token_ids
        )
    if args.warmup_only:
        fixed, fixed_meta = [], []
    else:
        fixed, fixed_meta = prepare_prompts(
            tokenizer, load_questions(args.fixed_data, args.fixed_count), args.input_tokens, args.send_token_ids
        )
    manifest = {
        "api_url": args.api_url,
        "model": args.model,
        ...
        "unique_cache_salt": args.unique_cache_salt,
        "send_token_ids": args.send_token_ids,
        "warmup": {"count": args.warmup_count, "output_tokens": args.warmup_output_tokens, "concurrency": 1, "prompts": warmup_meta},
        args.fixed_stage_name: {"count": args.fixed_count, "output_tokens": args.fixed_output_tokens, "concurrency": args.fixed_concurrency, "prompts": fixed_meta},
    ...
    (args.output_dir / "summary.json").write_text(
        json.dumps(
            {args.fixed_stage_name: fixed_summary} if args.fixed_only else {"warmup": warmup_summary, args.fixed_stage_name: fixed_summary},
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
    )
    print(f"{args.fixed_stage_name} finished", flush=True)
